...
SELECT app_name AS app FROM analytics WHERE id=1880;
...
SELECT app_name AS name, price, rating FROM analytics WHERE rating<3 ORDER BY price DESC LIMIT 1 ;

--Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.--

SELECT app_name AS name FROM analytics WHERE min_installs<=50 AND rating IS NOT NULL ORDER BY rating  DESC;

--Find the names of all apps that are rated less than 3 with at least 10000 reviews.--

SELECT app_name AS name FROM analytics where rating<3 and reviews>=10000;
...
SELECT SUM(reviews) AS reviews_sum FROM analytics;
